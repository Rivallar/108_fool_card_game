...
while st_screen.start_screen_flag:
	...
	gf.draw_text_line(st_screen.hint_font, st_screen.username_str, st_screen.text_color, username_field.left + 2, username_field.centery, screen)
...
#Creating players
players = []
players.append(Player(st_screen.username_str, bot=False))
...
turn = Turn(players[0])
gf.make_first_turn(turn, players, flags, active_deck, used_deck, queen_cards)			#first turn in each round starts with a random card of first player
...
while True:
	...
	if turn.player.bot and not flags.game_over_flag:
		gf.bot_turn(turn, bg_color, screen_rect, screen, players, back_img, used_deck, active_deck, right_arrow, flags, queen_cards, cancel_button, one_card_button)
				
	else:	
		gf.check_events(turn, used_deck, players, flags, active_deck, queen_cards, cancel_button, one_card_button, quit_button, play_again_button, play_deck)

	if not flags.end_game_flag:
		gf.draw_everything(bg_color, screen_rect, screen, players, back_img, used_deck, active_deck, right_arrow, flags, queen_cards, cancel_button, one_card_button)
