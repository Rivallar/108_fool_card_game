...
class Deck:
	...
	def give_card(self, player, quantity, used_deck, queen_cards, flags, reverse=False):
		...
		if cards_left == 0 and len(used_deck.cards) == 1:					# all cards in players hands
			return None
	...
	def reshuffle(self, player, quantity, used_deck, queen_cards, cards_left, flags):
		...
		player.round_points += 5 * flags.reshuffle_count					# penalty points for reshuffling. Should be taken from settings
...
class Player:
	...
	def use_card(self, card_id, used_deck):
		...
		elif self.hand[card_id].suit == used_deck.cards[-1].suit or self.hand[card_id].value == used_deck.cards[-1].value or self.hand[card_id].value == 'Q':
			used_deck.cards.append(self.hand.pop(card_id))
...
class GameFlags:
	...
	def __init__(self):
		...
		self.loose_points = 15											# points to loose the game should be taken from settings
...
class StartScreenProperties:
	...
	def __init__(self):
		...
		self.hints = hints = [
			...
			'> SPACEBAR to draw a card or pass your turn']
